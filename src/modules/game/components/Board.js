...
const Board = ({board, currentTurn, handleClick, possibleMoves, selected, prev, check, turn}) => <Stack border={1}>
  {
      ...
      {row.map((item, colNo) => {
        ...
        return <BoxContainer key={`${rowNo}_${colNo}`} onClick={handleClick(item)} isEven={(rowNo + colNo) % 2 === 0}
          ...
          <Image src={item ? images[`${item.color}${item.type}`.toUpperCase()] : null}
                 item={{...item, possibleMoves, selected, check: check && turn === item.color}} prev={prev}
          />
        </BoxContainer>
      })}
    </Stack>)
